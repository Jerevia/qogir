...
class JobRunner(object):
    ...
    def get_pyspark_command(self):
        ...
        spark_python_path = os.path.join(
            self.env, 'bin', 'python')
        if 'cdh' in self.config['command-params']['HADOOP_CONF_DIR'].lower():
            submit_exec_path = '/etc/alternatives/spark-submit'
        else:
    ...
    def _check_venv(self):
        ...
        if run_bash_command(_command) != 0:
            raise CommandError(
    ...
    def run(self, *args, **kwargs):
        ...
        self.configure()
        self._check_venv()
        if run_bash_command(self.command_) != 0:
            raise CommandError(
                'Command finished with a nonzero return code.')
